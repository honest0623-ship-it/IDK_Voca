...
def load_user_progress(username):
    ...
    # ✅ 숫자 변환 (TypeError 방지)
    for col in ['word_id', 'interval', 'fail_count']:
        if col in user_df.columns:
            user_df[col] = pd.to_numeric(user_df[col], errors='coerce')

    # 결측 처리 + 정수화
    if 'word_id' in user_df.columns:
        user_df = user_df.dropna(subset=['word_id'])
        user_df['word_id'] = user_df['word_id'].astype(int)
    else:
        user_df['word_id'] = pd.Series(dtype=int)

    if 'interval' in user_df.columns:
        user_df['interval'] = user_df['interval'].fillna(0).astype(int)
    else:
        user_df['interval'] = 0

    if 'fail_count' in user_df.columns:
        user_df['fail_count'] = user_df['fail_count'].fillna(0).astype(int)
    else:
        user_df['fail_count'] = 0
